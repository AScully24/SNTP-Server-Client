...
#include <unistd.h>
#include <time.h>
#include <arpa/inet.h> // Has function to convert ip address to an ASCII dot quad form
#include <netdb.h> // gethostbyadd()
#define MAXLINE 4096
#define MAXQ 10
...
int main(int argc, char * argv[]) {
	...
	servaddr.sin_addr.s_addr = htonl(INADDR_ANY); /* allow connection on any of my IP addresses - if I have more than one! */
	...
	for (;;) { /* loop forever */
		...
		//Client deals stored in cliaddr
		...
		// Use for host address
		hostptr = gethostbyaddr((char *) &cliaddr.sin_addr,
				sizeof(cliaddr.sin_addr), cliaddr.sin_family);
		...
		fprintf(stderr, "The client name is %s\n", hostptr->h_name);
                ...
                snprintf( buff, sizeof( buff), "%.24s\r\n", ctime( &ticks));
//		snprintf(buff, sizeof(buff), "OMG I ARE SERVER\n");
